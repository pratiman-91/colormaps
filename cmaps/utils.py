...
def concat(cnames, ratios, discrete, name=None, save=False):
    """
    ...
    for cname in cnames:
        ...
        if isinstance(cname, str):
            ...
            levels = np.array(tuple(np.linspace(start=0, stop=cname.N - 1, num=int(ratio * cname.N), dtype=int)))
            colors1 = cname.__getitem__(levels)
            colors1 = colors1.colors
            # combine them and build a new colormap
            colors_var = np.vstack((colors_var, colors1))

        j = j + 1
    if isinstance(cname, str):
        mymap = Colormap(colors_str[1:], name='add')
    else:
        mymap = Colormap(colors_var[1:], name='add')
