...
def _listfname():
    ...
    l.update(
        {
            "carbonplan": {
                "p": 'os.path.join(CMAPSFILE_DIR, "carbonplan", ',
                "l": sorted(glob(os.path.join(CMAPSFILE_DIR, "carbonplan/*.rgb"))),
            }
        }
    )
    return l
...
def write_cmaps(template_file="./cmaps.template"):
    ...
    for t in l.keys():
        for cmap_file in l[t]["l"]:
            ...
            c += '                if version.parse(mlp_version) >= version.parse("3.9.0"):\n'
            c += "                    return  matplotlib.colormaps[cname]\n"
            c += "                else: \n"
